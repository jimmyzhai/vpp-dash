...
dash_sai_create_flow_entry (const dash_header_t *dh)
{
    ...
    attrs[count].id = SAI_FLOW_ENTRY_ATTR_IS_UNIDIRECTIONAL;
    attrs[count++].value.booldata = flow_data->is_unidirectional;

    attrs[count].id = SAI_FLOW_ENTRY_ATTR_STATE;
    attrs[count++].value.u8 = SAI_DASH_FLOW_STATE_FLOW_CREATED;
    ...
    attrs[count].id = SAI_FLOW_ENTRY_ATTR_TUNNEL_ID;
    attrs[count++].value.u16 = ntohs(flow_data->tunnel_id);

    attrs[count].id = SAI_FLOW_ENTRY_ATTR_ROUTING_ACTIONS;
    attrs[count++].value.u32 = ntohl(flow_data->routing_actions);
    ...
    if ((u8*)(flow_overlay_data + 1) <= (u8*)dh + dh_length) {
        ...
        if (flow_overlay_data->is_ipv6) {
            ...
            attrs[count].id = SAI_FLOW_ENTRY_ATTR_OVERLAY_DATA_DIP;
            attrs[count].value.ipaddr.addr_family = SAI_IP_ADDR_FAMILY_IPV6;
            clib_memcpy_fast(attrs[count].value.ipaddr.addr.ip6,
        ...
        attrs[count].id = SAI_FLOW_ENTRY_ATTR_OVERLAY_DATA_SIP_MASK;
        attrs[count].value.ipaddr.addr_family = SAI_IP_ADDR_FAMILY_IPV6;
        clib_memcpy_fast(attrs[count].value.ipaddr.addr.ip6,
                         &flow_overlay_data->sip_mask, sizeof(sai_ip6_t));
        count++;

        attrs[count].id = SAI_FLOW_ENTRY_ATTR_OVERLAY_DATA_DIP_MASK;
        attrs[count].value.ipaddr.addr_family = SAI_IP_ADDR_FAMILY_IPV6;
        clib_memcpy_fast(attrs[count].value.ipaddr.addr.ip6,
    ...
    if ((u8*)(flow_encap_data + 1) <= (u8*)dh + dh_length) {
        ...
        attrs[count].id = SAI_FLOW_ENTRY_ATTR_ENCAP_DATA_DEST_VNET_VNI;
        attrs[count++].value.u32 = (flow_encap_data->dest_vnet_vni_high << 16) | ntohs(flow_encap_data->dest_vnet_vni_low);

        attrs[count].id = SAI_FLOW_ENTRY_ATTR_ENCAP_DATA_UNDERLAY_SIP;
        attrs[count++].value.u32 = ntohl(flow_encap_data->underlay_sip.as_u32);

        attrs[count].id = SAI_FLOW_ENTRY_ATTR_ENCAP_DATA_UNDERLAY_DIP;
        attrs[count++].value.u32 = ntohl(flow_encap_data->underlay_dip.as_u32);

        attrs[count].id = SAI_FLOW_ENTRY_ATTR_ENCAP_DATA_UNDERLAY_SMAC;
        clib_memcpy_fast(attrs[count].value.mac, flow_encap_data->underlay_smac, sizeof(ethernet_header_t));
        count++;

        attrs[count].id = SAI_FLOW_ENTRY_ATTR_ENCAP_DATA_UNDERLAY_DMAC;
        clib_memcpy_fast(attrs[count].value.mac, flow_encap_data->underlay_dmac, sizeof(ethernet_header_t));
        count++;

        attrs[count].id = SAI_FLOW_ENTRY_ATTR_ENCAP_DATA_DASH_ENCAPSULATION;
        attrs[count++].value.s32 = ntohs(flow_encap_data->dash_encapsulation);
    } else {
        u8 mac[6] = {0};
        /* set default value for bmv2 table */
        attrs[count].id = SAI_FLOW_ENTRY_ATTR_ENCAP_DATA_UNDERLAY_SMAC;
    ...
    if ((u8*)(flow_tunnel_data + 1) <= (u8*)dh + dh_length) {
        ...
        attrs[count].id = SAI_FLOW_ENTRY_ATTR_TUNNEL_DATA_DEST_VNET_VNI;
        attrs[count++].value.u32 = (flow_tunnel_data->dest_vnet_vni_high << 16) | ntohs(flow_tunnel_data->dest_vnet_vni_low);

        attrs[count].id = SAI_FLOW_ENTRY_ATTR_TUNNEL_DATA_UNDERLAY_SIP;
        attrs[count++].value.u32 = ntohl(flow_tunnel_data->underlay_sip.as_u32);

        attrs[count].id = SAI_FLOW_ENTRY_ATTR_TUNNEL_DATA_UNDERLAY_DIP;
        attrs[count++].value.u32 = ntohl(flow_tunnel_data->underlay_dip.as_u32);

        attrs[count].id = SAI_FLOW_ENTRY_ATTR_TUNNEL_DATA_UNDERLAY_SMAC;
        clib_memcpy_fast(attrs[count].value.mac, flow_tunnel_data->underlay_smac, sizeof(ethernet_header_t));
        count++;

        attrs[count].id = SAI_FLOW_ENTRY_ATTR_TUNNEL_DATA_UNDERLAY_DMAC;
        clib_memcpy_fast(attrs[count].value.mac, flow_tunnel_data->underlay_dmac, sizeof(ethernet_header_t));
        count++;

        attrs[count].id = SAI_FLOW_ENTRY_ATTR_TUNNEL_DATA_DASH_ENCAPSULATION;
        attrs[count++].value.s32 = ntohs(flow_tunnel_data->dash_encapsulation);
    } else {
        u8 mac[6] = {0};
        /* set default value for bmv2 table */
        attrs[count].id = SAI_FLOW_ENTRY_ATTR_TUNNEL_DATA_UNDERLAY_SMAC;
